...
\item
  ...
  \item
    ...
    al inicio de un vector en R es una operación costosa \(O(n)\) que
    puede impactar el rendimiento real de esta implementación en R. En
...
\item
  ...
  \item
    \textbf{Caso promedio}: \(O(n \log n)\). Con una buena elección de
    pivote (que divide el array en dos subproblemas de tamaño similar),
...
\item
  ...
  vencerás'' que ofrecen un rendimiento mucho mejor, generalmente
  \(O(n \log n)\).
  ...
  \item
    ...
    \(O(n \log n)\) en todos los casos, pero requiere \(O(n)\) de
    espacio adicional.
  \item
    \textbf{Quicksort} es típicamente el más rápido en la práctica
    (\(O(n \log n)\) promedio), especialmente en implementaciones
    ``in-place'', aunque su peor caso es \(O(n^2)\) y puede ser
    inestable. La elección del pivote es crítica para su eficiencia.
  \end{itemize}
\end{itemize}
...
\tightlist
\item
  \textbf{Complejidad Temporal (Worst Case):} \(O(n)\). En el peor de
  los casos (cuando el elemento no está presente o está al final), el
...
\subsubsection{Auto-balanceo
automático}\label{auto-balanceo-automuxe1tico}

La característica más definitoria de un árbol AVL es que se balancea
automáticamente después de cada inserción o eliminación. Ninguna
operación puede dejar al árbol en un estado desequilibrado de forma
permanente.(Adelson-Velskii y Landis 1962)
...
Si después de una inserción o eliminación algún nodo viola esta
condición, el árbol aplica rotaciones (simples o dobles) para restaurar
el equilibrio antes de finalizar la operación.(\textbf{brown2024avl?})

En Java puede implementarse así:

\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{int} \FunctionTok{factorBalance}\OperatorTok{(}\NormalTok{Nodo nodo}\OperatorTok{)} \OperatorTok{\{}
    \ControlFlowTok{if} \OperatorTok{(}\NormalTok{nodo }\OperatorTok{==} \KeywordTok{null}\OperatorTok{)} \OperatorTok{\{}
        \ControlFlowTok{return} \DecValTok{0}\OperatorTok{;}
    \OperatorTok{\}}
    \ControlFlowTok{return} \FunctionTok{altura}\OperatorTok{(}\NormalTok{nodo}\OperatorTok{.}\FunctionTok{izquierdo}\OperatorTok{)} \OperatorTok{{-}} \FunctionTok{altura}\OperatorTok{(}\NormalTok{nodo}\OperatorTok{.}\FunctionTok{derecho}\OperatorTok{);}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}

\#\#\#\#Altura garantizada O(log n)

En un árbol de búsqueda binario (BST) simple, insertar elementos en
orden creciente (1, 2, 3, 4, 5\ldots) produce una estructura degenerada
que es prácticamente una lista enlazada.(\textbf{nievergelt1972binary?})

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{BST degenerado:              AVL (mismos datos):}
...
Esto garantiza que, incluso en el peor caso, la altura del árbol nunca
supera aproximadamente 1.44 veces el logaritmo del número de nodos.

\subsubsection{\texorpdfstring{Rendimiento (O(\log n)) en todas las
operaciones
principales}{Rendimiento (O(n)) en todas las operaciones principales}}\label{rendimiento-on-en-todas-las-operaciones-principales}

Como consecuencia directa de la altura logarítmica garantizada, las tres
operaciones fundamentales tienen un costo predecible:
...
En un BST simple, el peor caso puede llegar a ser (O(n)). El árbol AVL
elimina ese riesgo al mantener siempre su estructura
balanceada.(\textbf{nievergelt1972binary?})

\subsubsection{Las cuatro rotaciones como mecanismo de
corrección}\label{las-cuatro-rotaciones-como-mecanismo-de-correcciuxf3n}

Cuando el factor de balance se rompe, el árbol AVL dispone de
exactamente cuatro tipos de rotación para restaurarlo. Cada una
corresponde a un patrón específico de desequilibrio:

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0741}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5309}}
...
\bottomrule\noalign{}
\endlastfoot
LL & Inserción en subárbol izquierdo-izquierdo & Rotación simple a la
derecha \\
RR & Inserción en subárbol derecho-derecho & Rotación simple a la
izquierda \\
LR & Inserción en subárbol izquierdo-derecho & Rotación doble (izq →
der) \\
RL & Inserción en subárbol derecho-izquierdo & Rotación doble (der →
izq) \\
\end{longtable}
}
...
\tightlist
\item
  \textbf{Ejemplo de Rotación Simple Derecha (caso LL):}
\end{itemize}

Supongamos que se insertan los valores 30, 20 y 10 en ese orden.

Esto provoca un desequilibrio de tipo LL (\emph{Left-Left}), porque la
inserción ocurre en el subárbol izquierdo del hijo
izquierdo.(Adelson-Velskii y Landis 1962)

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Antes de la rotación (desequilibrio LL):     Después de la rotación simple a la derecha:}
...
\tightlist
\item
  \textbf{Ejemplo de Rotación Simple Izquierda (caso RR):}
\end{itemize}

Supongamos que se insertan los valores 10, 20 y 30 en ese orden.

Esto provoca un desequilibrio de tipo RR (\emph{Right-Right}), porque la
inserción ocurre en el subárbol derecho del hijo
derecho.(\textbf{brown2024avl?})

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Antes de la rotación (desequilibrio RR):     Después de la rotación simple a la izquierda:}
...
\tightlist
\item
  \textbf{Ejemplo de Rotación Doble Izquierda-Derecha (caso LR):}
\end{itemize}

Supongamos que se insertan los valores 30, 10 y 20 en ese orden.

Esto provoca un desequilibrio de tipo LR (\emph{Left-Right}), porque la
inserción ocurre en el subárbol derecho del hijo izquierdo.(Amani et~al.
2015)

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Antes de la rotación (desequilibrio LR):     Después de la rotación doble (izq → der):}
...
\tightlist
\item
  \textbf{Ejemplo de Rotación Doble Derecha-Izquierda (caso RL):}
\end{itemize}

Supongamos que se insertan los valores 10, 30 y 20 en ese orden.

Esto provoca un desequilibrio de tipo RL (\emph{Right-Left}), porque la
inserción ocurre en el subárbol izquierdo del hijo derecho.(Amani et~al.
2015)

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Antes de la rotación (desequilibrio RL):     Después de la rotación doble (der → izq):}
...
\subsubsection{Árbol de búsqueda binaria (mantiene el
orden)}\label{uxe1rbol-de-buxfasqueda-binaria-mantiene-el-orden}

El árbol AVL hereda todas las propiedades de un árbol binario de
búsqueda (BST). Para cualquier nodo, todos los valores de su subárbol
izquierdo son menores y todos los valores de su subárbol derecho son
mayores.(\textbf{brown2024avl?})

Esto significa que un recorrido \emph{inorden} siempre produce los
elementos en orden ascendente, lo que lo hace especialmente útil para
búsquedas eficientes, consultas por rangos y recorridos
ordenados.(\textbf{nievergelt1972binary?})

\begin{itemize}
\tightlist
\item
  \textbf{Ejemplo de recorrido inorden:}
\end{itemize}
...
\subsubsection{Costo adicional: espacio y tiempo de
rotación}\label{costo-adicional-espacio-y-tiempo-de-rotaciuxf3n}

Los árboles AVL no son gratuitos. Cada nodo debe almacenar información
adicional, como su altura o su factor de balance, lo que implica un
pequeño costo extra de memoria en comparación con un BST
simple.(Adelson-Velskii y Landis 1962)

Además, cada operación de inserción o eliminación puede requerir
procesos de rebalanceo para mantener la propiedad AVL.(Adelson-Velskii y
Landis 1962)

\subsubsection{Costo extra por inserción y
eliminación}\label{costo-extra-por-inserciuxf3n-y-eliminaciuxf3n}

\begin{itemize}
\tightlist
\item
  \textbf{Actualizar alturas en el camino de regreso:} (O(\log n))
  pasos\\
\item
  \textbf{Rotaciones necesarias en inserción:} máximo 2 rotaciones\\
\item
  \textbf{Rotaciones necesarias en eliminación:} hasta (O(\log n))
  rotaciones
\end{itemize}

Aunque existe este costo adicional, el beneficio principal es garantizar
que las operaciones de búsqueda, inserción y eliminación mantengan un
rendimiento eficiente incluso en el peor caso.(Amani et~al. 2015)

\subsection{Estructura Fija de un
Nodo}\label{estructura-fija-de-un-nodo}

\subsection{Ejemplo en Java (Implementacion de un arbol
AVL)}\label{ejemplo-en-java-implementacion-de-un-arbol-avl}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{public class Node \{}
\NormalTok{    int key;}
\NormalTok{    int height;}
\NormalTok{    Node left, right;}

\NormalTok{    public Node(int key) \{}
\NormalTok{        this.key = key;}
\NormalTok{        this.height = 1;}
\NormalTok{    \}}
\NormalTok{\}}
\NormalTok{public class AVLTree \{}

\NormalTok{    private Node root;}

\NormalTok{    // ── Altura ──────────────────────────────────────}
\NormalTok{    private int height(Node n) \{}
\NormalTok{        return (n == null) ? 0 : n.height;}
\NormalTok{    \}}

\NormalTok{    private void updateHeight(Node n) \{}
\NormalTok{        n.height = 1 + Math.max(height(n.left), height(n.right));}
\NormalTok{    \}}

\NormalTok{    private int balanceFactor(Node n) \{}
\NormalTok{        return (n == null) ? 0 : height(n.left) {-} height(n.right);}
\NormalTok{    \}}

\NormalTok{    // ── Rotaciones ──────────────────────────────────}
\NormalTok{    private Node rotateRight(Node y) \{}
\NormalTok{        Node x  = y.left;}
\NormalTok{        Node T2 = x.right;}

\NormalTok{        x.right = y;}
\NormalTok{        y.left  = T2;}

\NormalTok{        updateHeight(y);}
\NormalTok{        updateHeight(x);}

\NormalTok{        return x;}
\NormalTok{    \}}

\NormalTok{    private Node rotateLeft(Node x) \{}
\NormalTok{        Node y  = x.right;}
\NormalTok{        Node T2 = y.left;}

\NormalTok{        y.left  = x;}
\NormalTok{        x.right = T2;}
...
\NormalTok{    // ── Rebalanceo ──────────────────────────────────}
\NormalTok{    private Node rebalance(Node n) \{}
\NormalTok{        updateHeight(n);}
\NormalTok{        int bf = balanceFactor(n);}

\NormalTok{        if (bf \textgreater{} 1 \&\& balanceFactor(n.left) \textgreater{}= 0)}
\NormalTok{            return rotateRight(n);}

\NormalTok{        if (bf \textgreater{} 1 \&\& balanceFactor(n.left) \textless{} 0) \{}
\NormalTok{            n.left = rotateLeft(n.left);}
\NormalTok{            return rotateRight(n);}
\NormalTok{        \}}

\NormalTok{        if (bf \textless{} {-}1 \&\& balanceFactor(n.right) \textless{}= 0)}
\NormalTok{            return rotateLeft(n);}

\NormalTok{        if (bf \textless{} {-}1 \&\& balanceFactor(n.right) \textgreater{} 0) \{}
\NormalTok{            n.right = rotateRight(n.right);}
\NormalTok{            return rotateLeft(n);}
\NormalTok{        \}}
...
\NormalTok{    // ── Insert ──────────────────────────────────────}
\NormalTok{    public void insert(int key) \{}
\NormalTok{        root = insertRec(root, key);}
\NormalTok{    \}}

\NormalTok{    private Node insertRec(Node n, int key) \{}
\NormalTok{        if (n == null) return new Node(key);}

\NormalTok{        if      (key \textless{} n.key) n.left  = insertRec(n.left,  key);}
\NormalTok{        else if (key \textgreater{} n.key) n.right = insertRec(n.right, key);}
\NormalTok{        else                  return n;}

\NormalTok{        return rebalance(n);}
\NormalTok{    \}}

\NormalTok{    // ── Search ──────────────────────────────────────}
\NormalTok{    public boolean search(int key) \{}
\NormalTok{        return searchRec(root, key);}
\NormalTok{    \}}

\NormalTok{    private boolean searchRec(Node n, int key) \{}
\NormalTok{        if (n == null)    return false;}
\NormalTok{        if (key == n.key) return true;}
\NormalTok{        if (key \textless{} n.key)  return searchRec(n.left,  key);}
\NormalTok{        else              return searchRec(n.right, key);}
\NormalTok{    \}}

\NormalTok{    // ── Delete ──────────────────────────────────────}
\NormalTok{    public void delete(int key) \{}
\NormalTok{        root = deleteRec(root, key);}
\NormalTok{    \}}

\NormalTok{    private Node deleteRec(Node n, int key) \{}
\NormalTok{        if (n == null) return null;}

\NormalTok{        if (key \textless{} n.key) \{}
\NormalTok{            n.left  = deleteRec(n.left,  key);}
\NormalTok{        \} else if (key \textgreater{} n.key) \{}
\NormalTok{            n.right = deleteRec(n.right, key);}
\NormalTok{        \} else \{}
\NormalTok{            if (n.left == null)  return n.right;}
\NormalTok{            if (n.right == null) return n.left;}

\NormalTok{            Node successor = minNode(n.right);}
\NormalTok{            n.key   = successor.key;}
\NormalTok{            n.right = deleteRec(n.right, successor.key);}
\NormalTok{        \}}

\NormalTok{        return rebalance(n);}
\NormalTok{    \}}

\NormalTok{    private Node minNode(Node n) \{}
\NormalTok{        while (n.left != null) n = n.left;}
\NormalTok{        return n;}
\NormalTok{    \}}

\NormalTok{    // ── Recorridos ──────────────────────────────────}
\NormalTok{    public void inOrder() \{}
\NormalTok{        System.out.print("In{-}Order:  [ ");}
\NormalTok{        inOrderRec(root);}
\NormalTok{        System.out.println("]");}
\NormalTok{    \}}

\NormalTok{    private void inOrderRec(Node n) \{}
\NormalTok{        if (n == null) return;}
\NormalTok{        inOrderRec(n.left);}
\NormalTok{        System.out.print(n.key + " ");}
\NormalTok{        inOrderRec(n.right);}
\NormalTok{    \}}

\NormalTok{    public void preOrder() \{}
\NormalTok{        System.out.print("Pre{-}Order: [ ");}
\NormalTok{        preOrderRec(root);}
\NormalTok{        System.out.println("]");}
\NormalTok{    \}}

\NormalTok{    private void preOrderRec(Node n) \{}
\NormalTok{        if (n == null) return;}
\NormalTok{        System.out.print(n.key + " ");}
\NormalTok{        preOrderRec(n.left);}
\NormalTok{        preOrderRec(n.right);}
\NormalTok{    \}}

\NormalTok{    public void printTree() \{}
\NormalTok{        System.out.println("Estructura del arbol:");}
\NormalTok{        printTreeRec(root, "", true);}
\NormalTok{    \}}

\NormalTok{    private void printTreeRec(Node n, String prefix, boolean isLeft) \{}
\NormalTok{        if (n == null) return;}
\NormalTok{        System.out.println(prefix + (isLeft ? "+{-}{-} " : "\textbackslash{}\textbackslash{}{-}{-} ") + n.key + " (h=" + n.height + ")");}
\NormalTok{        printTreeRec(n.left,  prefix + (isLeft ? "|   " : "    "), true);}
\NormalTok{        printTreeRec(n.right, prefix + (isLeft ? "|   " : "    "), false);}
\NormalTok{    \}}

\NormalTok{    public int treeHeight() \{}
\NormalTok{        return height(root);}
\NormalTok{    \}}
\NormalTok{\}}
\NormalTok{public class Main \{}

\NormalTok{    public static void main(String[] args) \{}

\NormalTok{        AVLTree avl = new AVLTree();}

\NormalTok{        System.out.println("==========================================");}
\NormalTok{        System.out.println("         ARBOL AVL {-} DEMOSTRACION        ");}
\NormalTok{        System.out.println("==========================================\textbackslash{}n");}

\NormalTok{        // {-}{-} Inserciones}
\NormalTok{        int[] valores = \{30, 20, 40, 10, 25, 35, 50, 5, 15, 28\};}

\NormalTok{        System.out.println("{-}{-} Insertando:");}
\NormalTok{        for (int v : valores) \{}
\NormalTok{            System.out.print("  insert(" + v + ")");}
\NormalTok{            avl.insert(v);}
\NormalTok{        \}}
\NormalTok{        System.out.println("\textbackslash{}n");}

\NormalTok{        avl.printTree();}
\NormalTok{        System.out.println();}
\NormalTok{        avl.inOrder();}
\NormalTok{        avl.preOrder();}
\NormalTok{        System.out.println("Altura del arbol: " + avl.treeHeight());}

\NormalTok{        // {-}{-} Busquedas}
\NormalTok{        System.out.println("\textbackslash{}n{-}{-} Busquedas:");}
\NormalTok{        int[] buscar = \{25, 99, 10, 1\};}
\NormalTok{        for (int k : buscar) \{}
\NormalTok{            System.out.printf("  search(\%2d) {-}\textgreater{} \%s\%n", k,}
\NormalTok{                avl.search(k) ? "ENCONTRADO" : "NO encontrado");}
\NormalTok{        \}}

\NormalTok{        // {-}{-} Eliminaciones}
\NormalTok{        System.out.println("\textbackslash{}n{-}{-} Eliminaciones:");}

\NormalTok{        System.out.println("  delete(20) [nodo con dos hijos]");}
\NormalTok{        avl.delete(20);}
\NormalTok{        avl.inOrder();}
\NormalTok{        avl.printTree();}

\NormalTok{        System.out.println("  delete(50) [hoja]");}
\NormalTok{        avl.delete(50);}
\NormalTok{        avl.inOrder();}

\NormalTok{        System.out.println("  delete(30) [raiz actual]");}
\NormalTok{        avl.delete(30);}
\NormalTok{        avl.inOrder();}
\NormalTok{        avl.printTree();}

\NormalTok{        System.out.println("  delete(99) [no existe]");}
\NormalTok{        avl.delete(99);}
\NormalTok{        avl.inOrder();}

\NormalTok{        System.out.println("\textbackslash{}nAltura final: " + avl.treeHeight());}
\NormalTok{        System.out.println("\textbackslash{}nTodas las operaciones completadas correctamente.");}
\NormalTok{    \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
Avanzadas''}\label{title-estructuras-avanzadas}
...
\section{Árboles AVL}\label{uxe1rboles-avl-1}
...
\subsection{Caracteristicas
Importantes}\label{caracteristicas-importantes-1}

\subsubsection{Auto-balanceo
automático}\label{auto-balanceo-automuxe1tico-1}
...
Si después de una inserción o eliminación algún nodo viola esta
condición, el árbol aplica rotaciones (simples o dobles) para restaurar
el equilibrio antes de finalizar la operación.
...
En un árbol de búsqueda binario (BST) simple, insertar elementos en
orden creciente (1, 2, 3, 4, 5\ldots) produce una estructura degenerada
que es prácticamente una lista enlazada.
...
\subsubsection{\texorpdfstring{Rendimiento \(O(\log n)\) en todas las
operaciones
principales}{Rendimiento O(\textbackslash log n) en todas las operaciones principales}}\label{rendimiento-olog-n-en-todas-las-operaciones-principales}
...
En un BST simple, el peor caso puede llegar a ser (O(n)). El árbol AVL
elimina ese riesgo al mantener siempre su estructura balanceada.

\subsubsection{Las cuatro rotaciones como mecanismo de
corrección}\label{las-cuatro-rotaciones-como-mecanismo-de-correcciuxf3n-1}
...
Esto provoca un desequilibrio de tipo LL (\emph{Left-Left}), porque la
inserción ocurre en el subárbol izquierdo del hijo izquierdo.
...
Esto provoca un desequilibrio de tipo RR (\emph{Right-Right}), porque la
inserción ocurre en el subárbol derecho del hijo derecho.
...
Esto provoca un desequilibrio de tipo LR (\emph{Left-Right}), porque la
inserción ocurre en el subárbol derecho del hijo izquierdo.
...
Esto provoca un desequilibrio de tipo RL (\emph{Right-Left}), porque la
inserción ocurre en el subárbol izquierdo del hijo derecho.
...
\subsubsection{Árbol de búsqueda binaria (mantiene el
orden)}\label{uxe1rbol-de-buxfasqueda-binaria-mantiene-el-orden-1}
...
Esto significa que un recorrido \emph{inorden} siempre produce los
elementos en orden ascendente, lo que lo hace especialmente útil para
búsquedas eficientes, consultas por rangos y recorridos ordenados.

\subsubsection{Árbol de búsqueda binaria (mantiene el
orden)}\label{uxe1rbol-de-buxfasqueda-binaria-mantiene-el-orden-2}
...
\subsubsection{Costo adicional: espacio y tiempo de
rotación}\label{costo-adicional-espacio-y-tiempo-de-rotaciuxf3n-1}

Los árboles AVL no son gratuitos. Cada nodo debe almacenar información
adicional, como su altura o su factor de balance, lo que implica un
pequeño costo extra de memoria en comparación con un BST simple.

Además, cada operación de inserción o eliminación puede requerir
procesos de rebalanceo para mantener la propiedad AVL.

\subsubsection{Costo extra por inserción y
eliminación}\label{costo-extra-por-inserciuxf3n-y-eliminaciuxf3n-1}
...
Aunque existe este costo adicional, el beneficio principal es garantizar
que las operaciones de búsqueda, inserción y eliminación mantengan un
rendimiento eficiente incluso en el peor caso.

\subsection{Estructura Fija de un
Nodo}\label{estructura-fija-de-un-nodo-1}

\subsection{Ejemplo en Java (Implementacion de un arbol
AVL)}\label{ejemplo-en-java-implementacion-de-un-arbol-avl-1}
...
\subsection{¿Por qué son
importantes?}\label{por-quuxe9-son-importantes-3}
...
\subsection{Árboles AVL y R}\label{uxe1rboles-avl-y-r-1}
...
\section{Árboles B}\label{uxe1rboles-b-1}
...
\subsection{Características Clave}\label{caracteruxedsticas-clave-1}
...
\subsection{¿Por qué son
importantes?}\label{por-quuxe9-son-importantes-4}
...
\subsection{Árboles B y R}\label{uxe1rboles-b-y-r-1}
...
\section{Fibonacci Heap}\label{fibonacci-heap-1}
...
\subsection{Operaciones y Complejidad
Amortizada}\label{operaciones-y-complejidad-amortizada-1}
...
\subsection{¿Por qué son
importantes?}\label{por-quuxe9-son-importantes-5}
...
\subsection{Fibonacci Heaps y R}\label{fibonacci-heaps-y-r-1}
...
\begin{quote}
\begin{quote}
27b65a7427acfcd52928a61765ee5f5125f8c0b3 Como expertos en R, nuestro
poder no reside solo en codificar estructuras de datos desde cero, sino
en saber cuándo y dónde se utilizan, cómo interactuar con ellas a través
de las abstracciones del lenguaje, y cómo elegir la herramienta adecuada
para cada problema, ya sea una función base de R, un paquete externo o
...
\bibitem[\citeproctext]{ref-adelson1962avl}
Adelson-Velskii, G. M., y E. M. Landis. 1962. {«An Algorithm for the
Organization of Information»}. \emph{Soviet Mathematics Doklady} 3:
1259-63. \url{https://zhjwpku.com/assets/pdf/AED2-10-avl-paper.pdf}.

\bibitem[\citeproctext]{ref-amani2015amortized}
Amani, Mahdi, Kevin A. Lai, y Robert E. Tarjan. 2015. \emph{Amortized
Rotation Cost in {AVL} Trees}.
\url{https://doi.org/10.48550/arXiv.1506.03528}.
